...
// smart_agent.js - Local Visual + Semantic Agent v0.10.18
...
(() => {
  ...
  class NodeScorer {
    static calculateProbability(candidate) {
      ...
      const codeTokens = (text.match(/[{};]|=>|\b(def|function|class|import|return)\b/g) || []).length;
      const align = candidate.bbox.x + candidate.bbox.w / 2;
      const viewportMid = (window.innerWidth || 1) / 2;
      const rightish = align > viewportMid * 1.1;
      const leftish = align < viewportMid * 0.95;
      const fileish = /(download|\.pdf|\.xlsx|\.pptx|\.zip|\.png|\.jpe?g|\.webp|\.md|\.txt|\.csv|\.json)\b/i.test(text)
        || candidate.signals.hasLink;
      ...
      if (fileish && candidate.signals.clickable) {
        type = 'FILE_CARD';
        confidence += 0.35;
        evidence.push('file_semantic_clickable');
      }
      if (mono || codeTokens >= 2 || candidate.signals.hasCode) {
        type = 'CODE_BLOCK';
        confidence = 0.9;
        evidence.push('monospace_or_code_tokens');
      }
      ...
      if (rightish) {
        role = 'user';
        evidence.push('alignment_right');
      } else if (leftish) {
        role = 'model';
        evidence.push('alignment_left');
      }

      if (type === 'MODEL_TURN' && role === 'user') type = 'USER_TURN';
      if (type === 'MODEL_TURN' && role === 'model') type = 'MODEL_TURN';
  ...
  class SmartAgent {
    ...
    static getVisualCandidates(rootEl, options = {}) {
      ...
      const elems = Array.from(rootEl.querySelectorAll('*')).slice(0, options.maxScan || 8000);
      for (const el of elems) {
        ...
        out.push({
          ...
          signals: signal,
          evidence: [bg !== bodyBg ? 'bg_differs_from_body' : 'bg_similar_body', signal.clickable ? 'clickable' : 'not_clickable']
        });
      }
      return out;
    }
