                ...
                if (ordered.length > 0) {
                    return {
                        ...
                        messages: ordered.map((n) => ({
                            ...
                            // Bug fix: parts can be objects (images/files), not just strings
                            content: ssotPartsToContent(n.message.content.parts),
...
// Bug fix: convert ChatGPT API content.parts (mixed string/object array) to rich content string.
// Object parts can be images (content_type: image_asset_pointer) or files (content_type: tether_id/attachment).
function ssotPartsToContent(parts) {
    const lines = [];
    for (const part of parts || []) {
        if (typeof part === 'string') {
            ...
            if (ct === 'image_asset_pointer' || ct.startsWith('image')) {
                const ptr = part.asset_pointer || '';
                const assetUrl = ptr ? `https://chatgpt.com/backend-api/files/${ptr.replace('file-service://', '')}` : '';
                if (assetUrl) lines.push(`[[IMG:${assetUrl}]]`);
            } else if (ct === 'tether_id' || ct === 'attachment' || ct === 'multimodal_text' || typeof part.tether_id === 'string') {
                const fileId = (part.tether_id || part.id || '').replace('file-service://', '');
                const fileName = part.name || part.filename || (fileId ? `file_${fileId.slice(0, 8)}.bin` : 'attachment.bin');
                const fileUrl = fileId ? `https://chatgpt.com/backend-api/files/${fileId}` : (part.url || '');
                if (fileUrl) lines.push(`[[FILE:${fileUrl}|${fileName}]]`);
            }
            // Other object parts (metadata, citations, etc.) — skip silently
        }
    }
    return lines.join('\n');
}
    ...
    try {
        ...
        return {
            ...
            messages: ordered.map((n) => {
                ...
                // tether_id = user-uploaded / AI-generated file (PDF, CSV, DOCX, etc.)
                // multimodal_text wraps file pointers in modern ChatGPT API versions
                const fileParts = parts.filter((p) => typeof p === 'object' && (
                    ...
                    (typeof p?.asset_pointer === 'string' && !p?.content_type?.startsWith('image'))
                ));
                ...
                for (const file of fileParts) {
                    // Bug fix: strip file-service:// prefix from tether IDs (same as image asset_pointer)
                    const fileId = (file.tether_id || file.id || '').replace('file-service://', '');
